...
export default function ExplorePage({params,}: {params: Promise<{ searchQuery: string }>;}) {
    ...
    const [selectedFilters, setSelectedFilters] = useState<Record<string, string[]>>({alcoholic:alcoholicJSON, category: [], spirit: [], glass:[] });
    ...
    const fetchByFilters = async () => {
        ...
        if(selectedFilters['category'].length > 0){
            for(var option of selectedFilters['category']){
                ...
                var ids = data.map(i => {return i.idDrink});
        ...
        var results = removeDuplicates(catResults.concat(spirResults.concat(glassResults)));
        if(results.length > 0){
      ...
      <section className="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4 pt-6">
        ...
        <form className="col-span-4 lg:col-span-1" role="search" aria-label="Filter drinks">
            <fieldset key="alcoholic">
                <Disclosure defaultOpen={true} key={alcoholics[0]} as="div" className="border border-gray-200 py-6 bg-white hover:bg-gray-50 rounded-lg p-4 mb-4">
                <h3>
                    Alcoholic content
                </h3>
                    <DisclosurePanel className="pt-6">
                        {alcoholics.map(option => {
                            return <div key={option} className="flex items-center">
                                <input onChange={() => handleFilterChange("alcoholic", option)} type='checkbox' defaultChecked={true} id={option} value={option} name="alcoholic-category" className="size-4 border-gray-300 text-indigo-600 focus:ring-indigo-500"/>
                                <label className="ml-3 text-sm text-gray-600" htmlFor={option}>{option}</label><br></br>
                            </div>
                        })}
                    </DisclosurePanel>
                </Disclosure>
            ...
            <fieldset key={section.id}>
                <Disclosure key={section.id} as="div" className="border border-gray-200 py-6 bg-white hover:bg-gray-50 rounded-lg p-4 mb-4">
                ...
                <DisclosurePanel className="pt-6">
                    ...
                    {section.options.map(option => (
                        ...
                        <input type="checkbox" id={option.value} checked={selectedFilters[section.id].includes(option.value)} onChange={() => handleFilterChange(section.id, option.value) } className="size-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"/>
        ...
        {/* Loading state */}
        {loading && (
          <div role="status" aria-live="assertive" className="col-span-4 h-screen flex items-center justify-center">
            <span className="sr-only">Loading search results...</span>
            <div className="loader" aria-hidden="true"></div>
          </div>
        )}
        {/* Results Grid */}
        <div ref={searchResultsContainerRef} className="lg:col-span-3 col-span-4">
